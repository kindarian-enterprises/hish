...
def setup_intelligence_collection():
    ...
    collection_name = os.getenv("INTELLIGENCE_COLLECTION_NAME", "cross_project_intelligence")
    model_name = os.getenv("EMBEDDING_MODEL", "BAAI/bge-small-en-v1.5")
    
    print(f"🧠 Setting up Cross-Project Intelligence Collection: {collection_name}")
    print(f"📡 Qdrant URL: {qdrant_url}")
    print(f"🤖 Embedding Model: {model_name}")
    
    try:
        # Initialize Qdrant client
        client = QdrantClient(url=qdrant_url, api_key=api_key if api_key else None)
        ...
        if collection_name in collection_names:
            ...
            recreate = input("Do you want to recreate it? (y/N): ").lower().strip()
            if recreate == 'y':
        ...
        print(f"\n🎯 Usage:")
        print(f"   - Framework docs: Use collection '{os.getenv('COLLECTION_NAME', 'hish_framework')}'")
