...
class TestIndexCompiler:
    ...
    def temp_hish_root(self):
        ...
        sample_index.write_text("""# Test Command Index
...
""")
    ...
    def test_compress_index_preserves_commands(self, temp_hish_root):
        ...
        assert "make new-context" in compressed_content or "newctx" in compressed_content
        assert "make list-contexts" in compressed_content or "make list-ctx" in compressed_content
    ...
    def test_exclusion_patterns_create_verbatim_copy(self, temp_hish_root):
        ...
        excluded_file = temp_hish_root / "local" / "workflow-indexes" / "dev_agent_persona.md"
...
"""
    ...
    def test_all_commands_preserved(self, sample_command_index):
        ...
        from sbmi.compiler.strategies import CombinedCompressionStrategy, StructuralCompressionStrategy, PhraseCompressionStrategy
...
class TestCacheAndIncrementalCompilation:
    ...
    def test_hash_computation(self, temp_hish_root_with_config):
        ...
        test_file = temp_hish_root_with_config / "local" / "workflow-indexes" / "test-index.md"
        cache_file = temp_hish_root_with_config / "local" / ".sbmi-cache.json"
        cache = CompilationCache(cache_file)
    ...
    def test_incremental_compilation(self, temp_hish_root_with_config):
        ...
        config = CompressionConfig()
        compiler = IndexCompiler(temp_hish_root_with_config, config=config, use_cache=True)
        ...
        compiler2 = IndexCompiler(temp_hish_root_with_config, config=config, use_cache=True)
        ...
        compiler3 = IndexCompiler(temp_hish_root_with_config, config=config, use_cache=True)
...
class TestConfigDrivenScanning:
    ...
    def temp_hish_with_structure(self):
        ...
        (temp_dir / "templates" / "style-and-philosophy" / "principles.md").write_text("# Principles\nContent")
    ...
    def test_scan_directories_from_config(self, temp_hish_with_structure):
        ...
        config = CompressionConfig()
        compiler = IndexCompiler(temp_hish_with_structure, config=config, use_cache=False)
    ...
    def test_is_excluded_checks_directories(self, temp_hish_with_structure):
        ...
        style_file = temp_hish_with_structure / "templates" / "style-and-philosophy" / "principles.md"
...
"""
    ...
    def test_verbatim_vs_compressed_distinction(self, temp_hish_root):
        ...
        assert compiler.processed_files["notes.md"]["strategy"] == "combined[structural,phrase]"

        # Excluded file should have verbatim strategy
        assert compiler.processed_files["dev_agent_persona.md"]["strategy"] == "verbatim_copy"
